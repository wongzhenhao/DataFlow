...
class VectorSQLGenerationPrompt:
    ...
    This class encapsulates various templates and criteria for generating
    SQL queries of different complexity levels, including support for
    standard SQL and vector-based (KNN) searches with SQLite-vec.
    """
    def __init__(self):
        """
        Initializes the SQLVecGenerationPrompt with predefined criteria and templates.
        """
        self.simple_criterion = '''**Criteria:**
        Simple SQL queries may satisfy one or more of the following criteria:
        - Simple queries should select data from a single table only.
    ...
    def sql_func_template(self, sql_funcs):
        """
        Formats the SQL functions into a template string.
        
        Args:
            sql_funcs (str): A string containing descriptions of SQL functions.
    ...
    def insert_stmts_template(self, insert_statements):
        """
        Formats the INSERT statements into a template string.
        
        Args:
            insert_statements (str): A string containing example INSERT INTO statements.
            
        Returns:
            str: A formatted string containing the INSERT statements.
        """
        template = '''### INSERT INTO Statements
        Below are several `INSERT INTO` statements. Use these to help generate predicates (i.e., `WHERE` clauses) in your SQL query:
        {insert_statements}
    ...
    def sql_synthesis_prompt(self, schema_str, sql_function_prompt, db_value_prompt, complexity, criterion, db_engine, column_count, db_extension, embedding_model):
        """
        Generates the final SQL synthesis prompt by combining all components.
        
        Args:
            schema_str (str): The database schema definition.
            sql_function_prompt (str): The prompt section for SQL functions.
            db_value_prompt (str): The prompt section for database values (INSERT statements).
            complexity (str): The desired complexity level of the SQL query.
            criterion (str): The criteria defining the complexity level.
            db_engine (str): The target database engine (e.g., "SQLite").
            column_count (int): The required number of columns in the SELECT statement.
            db_extension (str): The specific database extension to be used (e.g., "SQLite-vec").
            embedding_model (str): The name of the embedding model for vector search.
            
        Returns:
            str: The complete, formatted prompt for SQL query generation.
        """
        template = '''**Task Overview**
        Create an executable SQL query based on the provided information.
    ...
    def sqlite_funcs(self):
        """
        Returns a list of available SQLite functions with their descriptions.
        
        Returns:
            list: A list of strings, where each string describes an SQLite function.
        """
        funcs = [
            "ABS(X) \nDescription: The ABS(X) function returns the absolute value of the numeric argument X. Abs(X) returns NULL if X is NULL. Abs(X) returns 0.0 if X is a string or blob that cannot be converted to a numeric value. If X is the integer -9223372036854775808 then ABS(X) throws an integer overflow error since there is no equivalent positive 64-bit two complement value. ",
            "CHANGES() \nDescription: The CHANGES() function returns the number of database rows that were changed or inserted or deleted by the most recently completed INSERT, DELETE, or UPDATE statement, exclusive of statements in lower-level triggers. The CHANGES() SQL function is a wrapper around thesqlite3_changes64()C/C++ function and hence follows the same rules for counting changes. ",
...
class Text2VecSQLQuestionGeneratorPrompt:
    ...
    def build_prompt(self, data, input_db_id_key, input_sql_key, styles, db_id2column_info, db_type, using_sqlite_vec, extension) -> str:
        style_name = random.sample(styles, 1)[0]
        column_name2column_desc = db_id2column_info[data[input_db_id_key]]
        used_column_name2column_desc = dict()
