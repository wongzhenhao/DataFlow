...
class AnswerModelJudge(OperatorABC):
    ...
    def get_desc(lang: str = "zh"):
        if lang == "zh":
            return (
                ...
                "调用大语言模型进行语义理解和判断，最终返回每个答案是否正确的二分类结果（保留正确答案的样本）。\n"
                "输入参数：\n"
            ...
            return (
                ...
                "and judgment, ultimately returning a binary classification result for each answer (retaining samples with correct answers).\n"
                "Input Parameters:\n"
    ...
    def run(self, storage: DataFlowStorage, question_key: str = "question", answer_key: str = "answer", reference_key: str = "reference_answer") -> list:
        ...
        if len(valid_rows) == 0:
            ...
            output_file = storage.write(pd.DataFrame(columns=dataframe.columns))
            self.logger.info(f"Empty dataframe saved to {output_file}. Skipped {skipped_count} samples due to missing reference answers.")
            return required_columns
        ...
        # 保留results为True的行（答案正确的样本）
        filtered_dataframe = valid_rows.iloc[result_mask]
        output_file = storage.write(filtered_dataframe)
        
        # 记录统计信息
        total_samples = len(dataframe)
        valid_samples = len(valid_rows)
        correct_samples = len(filtered_dataframe)
        accuracy = correct_samples / valid_samples if valid_samples > 0 else 0
        
        self.logger.info(f"Filtered answers saved to {output_file}.")
        self.logger.info(f"Total samples: {total_samples}, Valid samples: {valid_samples}, Skipped samples: {skipped_count}")
        self.logger.info(f"Correct answers: {correct_samples}, Accuracy: {accuracy:.2%}")
