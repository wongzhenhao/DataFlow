    ...
    def get_desc(lang: str = "zh"):
        if lang == "zh":
            return (
                ...
                "  - key：要过滤的字段名\n"
                "  - op：过滤操作符，如 '==', '>=', 'in', 'range' 等\n"
                "  - value：用于比较的值或范围（如 [0.8, 1.0]）\n\n"
                "支持的操作符包括：==, !=, >, >=, <, <=, in, not in, contains, startswith, endswith, isna, notna, range"
            )
        elif lang == "en":
            return (
                ...
                "string matching, and missing value checks.\n\n"
                "Input Parameters:\n"
                "- filter_rules: A list of filter rules. Each rule must include:\n"
                "  - key: The field name to filter on\n"
                "  - op: The operator, such as '==', '>=', 'in', 'range', etc.\n"
                "  - value: The comparison value or range (e.g., [0.8, 1.0])\n\n"
                "Supported operators: ==, !=, >, >=, <, <=, in, not in, contains, startswith, endswith, isna, notna, range"
            )
        else:
            return "FlexibleFilter applies advanced filtering rules on DataFrame columns using a variety of operations."
