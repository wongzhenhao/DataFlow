...
class VecSQLExecutionClassifier(OperatorABC):
    def __init__(self,
        ...
        if self.num_generations <= self.difficulty_config["thresholds"][-1]:
            ...
            self.logger.warning(
                f"num_generations is less than the last threshold, will be set to {nearest_multiple}")
    ...
    def get_desc(lang):
        if lang == "zh":
            return (
                "让模型根据自然语言问题、数据库Schema和提示词，多次生成VecSQL，通过生成SQL的准确率，评估该问题对于模型的难度。\n\n"
                "输入参数：\n"
            ...
            return (
                "This operator evaluates the difficulty of VecSQL generation for a question based on the accuracy of generated SQLs.\n\n"
                "Input parameters:\n"
    ...
    def check_column(self, dataframe):
        required_columns = [self.input_db_id_key,
                            self.input_sql_key, self.input_prompt_key]
        missing_columns = [
            col for col in required_columns if col not in dataframe.columns]
        if missing_columns:
            raise ValueError(f"Missing required columns: {missing_columns}")

    @staticmethod # <-- 增加 @staticmethod
        ...
        try:
            ...
            for i, (predicted_sqls, _, _, idx) in enumerate(zip(predicted_sqls_list, ground_truth_list, db_ids, idxs)):
                ...
                for predicted_sql in predicted_sqls:
                    result_data.append(
                        {'res': 0, 'sql': predicted_sql, 'error': 'batch_execution_failed'})
                results.append(
                    {"idx": idx, "cnt_true": -1, "results": result_data})
            return results
    ...
    def run_sqls_parallel(self, datas, database_manager, num_cpus, meta_time_out):
        ...
        def process_batch(batch_data):
            ...
            # 核心修改：使用正确的类名 VecSQLExecutionClassifier
            return VecSQLExecutionClassifier.execute_model_batch(
        ...
        with ThreadPoolExecutor(max_workers=num_cpus) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    self.logger.warning(
                        f"Error in batch SQL execution: {e}")
    ...
    def report_statistics(self, dataframe: pd.DataFrame):
        ...
        stats = [f"{difficulty.title()}: {counts.get(difficulty, 0)}" for difficulty in [
    ...
    def run(self, storage: DataFlowStorage,
        ...
        self.logger.info(
            f"Processing {len(input_prompts)} questions, generating {self.num_generations} SQLs each...")
        ...
        for i, data in enumerate(datas):
            ...
            for response in question_responses:
                if response:
                    # 使用 self 调用，因为 parse_response 不再是静态的
        ...
        for execres in exec_result:
            if execres is not None:
                ...
                datas[idx][self.output_difficulty_key] = self.classify_difficulty(
                    cnt_true)
        ...
        self.logger.info(
            f"Difficulty classification results saved to {output_file}")
